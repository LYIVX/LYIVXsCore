...
import net.lyivx.ls_core.client.gui.layouts.SidebarSplitLayout;

/**
 * Base screen using SidebarSplitLayout (left sidebar + two columns on the right).
 * Implementors must provide addContents(); header/footer are provided.
 */
@Environment(EnvType.CLIENT)
public abstract class SidebarSplitSubScreen extends Screen {
    protected final Screen lastScreen;
    protected final Options options;
    public final SidebarSplitLayout layout = new SidebarSplitLayout(this);

    private int navScroll = 0;
    private int navContentHeight = 0;

    public SidebarSplitSubScreen(Screen lastScreen, Options options, Component title) {
        super(title);
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
        ...
        int separatorX = this.layout.getRightColumnSeparatorX();
        int contentTop = this.layout.getContentTop();
        int contentBottom = this.layout.getFooterTop();
        guiGraphics.fill(separatorX, contentTop, separatorX + 1, contentBottom - 30, 0x55000000);
    ...
    protected void setNavScroll(int scroll) {
        this.navScroll = scroll;
    }

    protected void setNavContentHeight(int height) {
        this.navContentHeight = height;
    }

    protected int getNavScroll() {
        return this.navScroll;
    }

    protected int getNavContentHeight() {
        return this.navContentHeight;
    }
